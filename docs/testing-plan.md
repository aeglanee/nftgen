...
| B23 | flowtable smoke | long-lived flow with `flow-offload` | flow uninterrupted; `nft list flowtables` shows ft; no functional regression |
...
1. ~~Harness fixture (`netns` builder + apply/teardown) with B01–B03 to prove
   ...
   **B04–B12 landed 2026-07-10** (ct invalid via raw ACK + counter; vmap
   zone/pair/group dispatch; named-set membership; bogon scrub + counter;
   concat no-bleed; live blocklist add/block/expire).
