...
- **Run tests:** `python -m pytest`. (Own venv + lint suite being set up;
  until then dev borrows the aerleon venv.)
...
Pre-commit hooks and the lint suite are being adopted now (mirroring
sessrumnir main). Still later: `AGENTS.md` conventions and CI wiring, when we
wire nftgen toward the collection.
