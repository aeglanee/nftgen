...
def test_single_check_explicit_mask():
    assert R.render(
        {"flags": {"match": ["syn"], "mask": ["syn", "ack"]}, "action": "accept"}
...
def test_subset_violation_errors():
    with pytest.raises(BuildError):
        R.render(
            {"flags": {"match": ["syn", "fin"], "mask": ["syn"]}, "action": "drop"}
        )
...
def test_unknown_flag_errors():
    with pytest.raises(BuildError):
        R.render(
            {"flags": {"match": ["syn", "bogus"], "mask": "all"}, "action": "drop"}
        )
