...
def test_v6_family():
    s = build_sets(
        [{"name": "f", "concat": ["saddr", "daddr"], "tuples": [["v6", "v6"]]}], DEFS
    )[0]
...
def test_multivalue_field_errors():
    with pytest.raises(BuildError):
        build_sets(
            [{"name": "f", "concat": ["saddr", "daddr"], "tuples": [["grp", "db"]]}],
            DEFS,
        )
...
def test_unknown_field_errors():
    with pytest.raises(BuildError):
        build_sets(
            [{"name": "f", "concat": ["saddr", "bogus"], "tuples": [["app", "x"]]}],
            DEFS,
        )
...
def test_mixed_family_errors():
    with pytest.raises(BuildError):
        build_sets(
            [{"name": "f", "concat": ["saddr", "daddr"], "tuples": [["app", "v6"]]}],
            DEFS,
        )
        ...
        [
            {
                ...
                "concat": ["saddr", "daddr", "dport"],
                "proto": "tcp",
                "tuples": [["app", "db", "postgres"], ["iot", "web", "https"]],
            }
        ],
        DEFS,
    )
