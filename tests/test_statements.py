...
def test_limit():
    assert one({"proto": "tcp", "dport": "ssh", "ct": ["new"], "limit": "10/minute", "action": "accept"}) == \
...
def test_log_bool_and_opts():
    assert one({"proto": "tcp", "dport": "ssh", "log": True, "action": "accept"}) == \
        "tcp dport @ssh log accept"
    assert one({"proto": "tcp", "dport": "ssh", "log": {"prefix": "ssh ", "level": "info"}, "action": "accept"}) == \
...
def test_statement_order_before_verdict():
    line = one({"proto": "tcp", "dport": "ssh", "limit": "5/second", "log": True, "counter": True, "action": "accept"})
